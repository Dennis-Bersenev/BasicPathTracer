...
public:
	vec3 center;
	vec3 disk_n;
	float r_tube, r_disk, R;
	material* mat;

	torus() {}

	//n must be unit length, r1 is dist from center to medial axis, r2 is dist from medial axis to surface.
	torus(vec3 c, vec3 n, float r1, float r2, material* mat) : center{ c }, disk_n{ n }, r_disk{ r1 }, r_tube{ r2 }, mat{ mat }
	{
		R = r1 + r2;
	}
	...
	inline float distance(const ray& r, vec3& m) const
	{
		float k = dot((r.origin() - center), disk_n);
	...
	inline float max_distance(const ray& r) const
	{
...
bool torus::hit(const ray& r, float t_min, float t_max, hit_record& rec) const
{
	
	float dsf = 0.0f, eps = 0.0001, max_dist = max_distance(r), radius;
	vec3 m;
